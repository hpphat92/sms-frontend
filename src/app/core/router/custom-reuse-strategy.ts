...
export class CustomReuseStrategy implements RouteReuseStrategy {
  ...
  public addStoredRoute(key, storedRoute: RouteStorageObject) {
    this.storedRoutes[key] = storedRoute;
    this._routerService.storedRoutes = this.storedRoutes;
  }

  public removeStoredRoute(key) {
   ...
   * that the user is currently on, and we would like to know if we want to store it
   * @returns boolean indicating that we want to (true) or
   * do not want to (false) store that route
   */
   ...
   * and offered up to whatever controller is using this class
   */
  public store(route: ActivatedRouteSnapshot, handle: DetachedRouteHandle): void {
    ...
    // and the handle is stored under it so that
    // you can only ever have one object stored for a single path
  ...
  public shouldAttach(route: ActivatedRouteSnapshot): boolean {
    ...
    // in place of the requested route, and is the return value
    // at this point we already know that the paths match because
    // the storedResults key is the route.routeConfig.path
    ...
    if (canAttach) {
      ...
      let queryParamsMatch: boolean = this.compareObjects(route.queryParams,
        storedRoute.snapshot.queryParams);

      // console.log("deciding to attach...", route, "does it match?",
      // storedRoute.snapshot, "return: ", paramsMatch && queryParamsMatch);
  ...
  public retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle {
    ...
    // return null if the path does not have a routerConfig OR
    // if there is no stored route for that routerConfig
    if (!route.routeConfig || !this.storedRoutes[keyPath]) {
      return null;
    }
  ...
  public shouldReuseRoute(future: ActivatedRouteSnapshot, curr: ActivatedRouteSnapshot): boolean {
    // console.log("deciding to reuse", "future", future.routeConfig,
    // "current", curr.routeConfig, "return: ", future.routeConfig === curr.routeConfig);
   ...
   * One thing to note is that it uses coercive comparison (==) on properties
   * which both objects have,
   * not strict comparison (===)
   * @param base The base object which you would like to compare another object to
   * @param compare The object to compare to base
   * @returns boolean indicating whether or not the objects have all the same properties
